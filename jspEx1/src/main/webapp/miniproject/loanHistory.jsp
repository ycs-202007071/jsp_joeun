...
<body>
    ...
    try {
        ...
        %>
        <table border="1">
            <caption>대출 목록</caption>
            <%
            for (String bookNum : bookNumbersList) {
                // 대출 기록 쿼리 수정
                String loanQuery = "SELECT * FROM loans WHERE bookNum = '" + bookNum + "' AND ID = '" + id + "'";
                rsLoan = stmt.executeQuery(loanQuery);
                boolean isLoaned = rsLoan.next(); // 유저가 대출한 책인지 확인
                ...
                if (isLoaned) {
                    ...
                    if (rs.next()) {
                        ...
                        <tr>
                            ...
                            <td>
                                <% if (isLoaned) { %>
                                    <button onclick="fnLoan('<%= bookNum %>')">반납</button>
                                <% } %>
                            </td>
                        </tr>
                        <%
...
function fnLoan(bookNum) {
    window.open("return.jsp?booknum=" + bookNum, "loan", "width=500,height=300");
}
</script>
